...
// see http://tools.ietf.org/html/draft-kelly-json-hal-06#section-8.2
    ...
    internals.link = function (link, relativeTo) {
        ...
        if (relativeTo && (internals.isRelativePath(link.href) || settings.absolute || !link.href.includes(relativeTo))) {
            link.href = internals.buildUrl(undefined, (url.resolve(relativeTo+'/', link.href)));
        }
        return link;
    };
...
// keeps found routes in a cache
    ...
    internals.route = function (routeId, params) {
        var route = selection.lookup(routeId) || internals.locateRoute(routeId);
    ...
    internals.halifyAndConfigure = function(rep, config, callback) {
        ...
        var configureEntity = function(rep, callback) {
    ...
    internals.buildUrl = function (request, path, search) {
        return url.format({
            host: settings.host || _.get(request, ['headers', 'host'], undefined),
            hostname: settings.hostname || _.get(request, ['connection', 'info', 'host'], undefined),
            port: settings.port || _.get(request, ['connection', 'info', 'port'], undefined),
            pathname: path.replace('%7B', '{').replace('%7D', '}'),
            protocol: settings.protocol || _.get(request, ['connection', 'info', 'protocol'], undefined),
            search: search
        }).replace(/{(.*?)}/g, (param)=>decodeURIComponent(param));
    ...
    internals.getRequestPath = function (request, queryTemplate, absolute) {
        var uriTemplate;
        request.path = request.path.split('/').map((component)=>encodeURIComponent(component)).join('/');
    ...
    internals.location = function (request, uri, absolute) {
        ...
        path = path.split('/').map((component)=>encodeURIComponent(component)).join('/');
        var search = null;
    ...
    internals.postHandler = function (request, reply) {
        ...
        if (mediaType && internals.shouldHalify(request)) {
            ...
            request.server.log(['debug'], 'Halacious post handler start');
            ...
            internals.halifyAndConfigure(rep, halConfig, function (err, rep) {
                if (err) {
                    return reply(err);
                }
                ...
                request.server.log(['debug'], 'Halacious post handler end');
    ...
    // Register the handler on the server (possibly with selected connections as specified
    // in the plugin options call to server.register().)
    server.ext('onPostHandler', internals.postHandler);
